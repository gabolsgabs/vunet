...
def update_cond_shape(num_frames):
    reshape = False
    conditions_shape = {"phonemes": 40, "phoneme_types": 9, "notes": 97, "chars": 29}
...
def istft(data):
    return librosa.istft(
        data, hop_length=config_prepro.HOP, win_length=config_prepro.FFT_SIZE
    )


def adapt_pred(pred, target):
    # normalization between target range
    pred = (np.max(target) - np.min(target)) * (
...
def prepare_a_condition(cond, num_frames):
    ...
    for ndx, i in enumerate(np.arange(0, size, num_frames - config.OVERLAP)):
        segment = np.zeros((num_bands, num_frames, cond.shape[-1]), dtype=np.float32)
...
def separate_audio(path_audio, path_output, model, cond):
    y, _ = analize_spec(spec_complex(path_audio)["spec"], model, cond)
...
def analize_spec(model, orig_mix_spec, cond):
    ...
    try:
        ...
        pred_mag = np.squeeze(concatenate(pred_mag, orig_mix_spec.shape), axis=-1)
    ...
    for t, i in enumerate(cond):
        ...
        if len(pos) > 0:
            ...
            for j in np.where(np.diff(pos) != 1)[0]:
                ...
                segments[tmp]["dur"] = end / config_prepro.FR - init / config_prepro.FR
...
def do_an_exp(model, data):
    ...
    # mix = istft(data['mix'])
    # acc = istft(data['acc'])
    target = istft(np.multiply(data["vocals"], data["normalization"]["vocals"][0]))
    mix = istft(np.multiply(data["mix"], data["normalization"]["mixture"][0]))
    acc = istft(np.multiply(data["acc"], data["normalization"]["accompaniment"][0]))
    ...
    sdr, sir, sar, perm = mir_eval.separation.bss_eval_sources(
        reference_sources=orig, estimated_sources=pred, compute_permutation=False
    )
...
def create_pandas(files, features):
    ...
    if config.PER_FEATURE:
        columns += [
            j
            for i in range(features + 1)  # for no silence feature
            for j in [
                "sdr_" + str(i),
                "sir_" + str(i),
                "sar_" + str(i),
...
def load_checkpoint(path_results):
    from vunet.train.models.unet import unet_model
...
def load_a_unet():
    ...
    if config.MODE == "standard":
        ...
        model_type = "_".join([config.CONDITION, config.FILM_TYPE, config.CONTROL_TYPE])
