...
def eval_silent_frames(
    true_source, predicted_source, window_size: int, hop_size: int,
    eval_incomplete_last_frame=False, eps_for_silent_target=True
):
    """
    ...
    number_eval_frames = int(np.ceil(
        (len(true_source)-window_size)/hop_size)) + 1
    ...
    for n in range(number_eval_frames):
        ...
        if eps_for_silent_target:
            ...
            if sum(abs(prediction_window)) <= thd_pred and sum(abs(true_window)) > thd_true:
                true_source_energy_at_silent_prediction = 10*np.log10(
