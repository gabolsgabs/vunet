...
def apply_to_keys(keys, func, *args, **kwargs):
    def wrapped(d):
        return {k: func(v, *args, **kwargs) if k in keys else v for k, v in d.items()}
...
def get_data(data):
    def py_get_data(uid, ndx, val_set):
        ...
        inputs = get_input_frame(target, DATA[uid], ndx, val_set).astype(np.float32)
    ...
    target, input, conditions = tf.py_function(
        py_get_data,
        [data["uid"], data["index"], data["val_set"]],
...
def dataset_generator(val_set=False):
    ...
    ds = tf.data.Dataset.from_generator(
        ...
        args=[val_set],
    ).map(get_data, num_parallel_calls=config.NUM_THREADS)
    if config.MODE in ["conditioned", "attention"]:
        ds = ds.map(
    ...
    ds = (
        ds.map(convert_to_estimator_input, num_parallel_calls=config.NUM_THREADS)
        .batch(config.BATCH_SIZE, drop_remainder=True)
        .prefetch(buffer_size=config.N_PREFETCH)
    )
    if not val_set:
        ds = ds.repeat()
    return ds
