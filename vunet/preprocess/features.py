...
def get_config_as_str():
    return {"FR": config.FR, "FFT_SIZE": config.FFT_SIZE, "HOP": config.HOP}
...
def compute_target(entry, dur, time_r, t):
    ...
    if t == "chars" or t == "notes":
        g = "notes"
    return entry.get_annot_as_matrix(time_r, dur=dur, t=t, g=g).astype(np.int16)
...
def get_a_song(entry, folder):
    ...
    d_target = {key: compute_target(entry, dur, time_r, key) for key in config.TARGETS}
